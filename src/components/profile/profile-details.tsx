...
import { MapPin, Leaf, Settings, UserPlus, MessageSquare, Edit3 } from 'lucide-react';
...
export function ProfileDetails({ user, isCurrentUser = false }: ProfileDetailsProps) {
  ...
  return (
    <>
      <div className="bg-card text-card-foreground rounded-xl shadow-lg overflow-hidden p-4 sm:p-6 relative">
        {isCurrentUser && (
          <Link href="/settings/account" className="absolute top-4 right-4 z-10">
        ...
        <div className="flex flex-row items-center gap-4 sm:gap-6 mb-4">
          ...
          <div className="hidden sm:flex flex-1 justify-around items-center text-center pt-2">
            <div>
            ...
            <Link href={`/profile/${user.id}/following`} className="hover:opacity-80 transition-opacity">
        ...
        <div className="text-left"> {/* Changed from text-center sm:text-left */}
          <h1 className="text-md font-semibold text-foreground">{user.name || user.username}</h1>
          <p className="text-sm text-muted-foreground">@{user.username}</p>
          
          {user.bio && <p className="text-sm text-foreground/90 leading-relaxed mt-2">{user.bio}</p>}
          
          <div className="flex flex-col sm:flex-row flex-wrap items-start justify-start gap-x-3 gap-y-1 text-xs text-muted-foreground mt-2">
            {user.location && (
        ...
        {/* Stats for smaller screens - below info */}
        <Separator className="my-4 sm:hidden" />
        <div className="flex sm:hidden justify-around items-center text-center w-full pt-1">
            <div>
              <p className="text-md font-semibold text-foreground">{user.postCount ?? 0}</p>
              <p className="text-xs text-muted-foreground">Posts</p>
            </div>
            <div>
              <p className="text-md font-semibold text-foreground">{user.followersCount ?? 0}</p>
              <p className="text-xs text-muted-foreground">Followers</p>
            </div>
            <Link href={`/profile/${user.id}/following`} className="hover:opacity-80 transition-opacity">
              <p className="text-md font-semibold text-foreground">{user.followingCount ?? 0}</p>
              <p className="text-xs text-muted-foreground">Following</p>
            </Link>
        </div>

        {/* Action Buttons Section - only for non-current users now */}
        {!isCurrentUser && (
