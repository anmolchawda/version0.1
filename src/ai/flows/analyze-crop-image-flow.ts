'use server';
/**
 * @fileOverview A Genkit flow for analyzing crop images to detect diseases, nutrient deficiencies, or insects.
 *
...
const AnalysisDetailsSchema = z.object({
  ...
  name: z.string().optional().describe('The common name of the detected issue (e.g., "Powdery Mildew", "Nitrogen Deficiency", "Aphids").'),
...
const AnalyzeCropImageOutputSchema = z.object({
  ...
  suggestions: z.array(z.string()).optional().describe("Brief, actionable suggestions if issues are detected (e.g., 'Consider applying a fungicide', 'Check soil pH').")
});
...
6.  Provide an 'overallAssessment' of the plant's condition. If no plant is detected, state that. If a plant is detected but appears healthy, state that. Otherwise, summarize the findings.
7.  If issues are detected, provide 1-2 brief, actionable 'suggestions'.

Return your analysis strictly in the specified JSON output format.
If no specific issue (disease, deficiency, insect) is detected for a category, set 'detected' to false and omit 'name', 'description', and 'confidence' for that category.
`,
});
